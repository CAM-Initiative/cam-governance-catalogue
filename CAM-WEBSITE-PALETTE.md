...
Run `pnpm run validate:palette`. The validator ensures the Corpus surface tokens exist, that both the home-page Constitutional Interfaces and the Evidence-to-Repair report use them, and that a hard-coded green/cyan hue cannot be introduced into the report header. It also runs in the published-site validation workflow.
