...
function deriveRecordChain(record: UnknownRecord, recordType: string, id: string): RecordChain {
  ...
  // Evidence origins remain visible, but contextual links are deliberately excluded.
  ...
  // `repair_scope` is authoritative where present. Older records retain a narrow
  // compatibility path until they have been migrated, but `contextual_relations`
  // can never enter this traversal.
  if (repairScope) {
