...
const routes: RuntimeRoute[] = [
  {
    ...
    phases: runtimePhases.map((phase) => ({ sublabel: phase.sublabel, detail: phase.technical, state: phase.summary })),
  },
  {
    id: "itz",
    label: "ITZ Example",
  ...
  },
  {
    ...
    phases: [
      ...
      { sublabel: "Relational clarification", detail: "Consent is surfaced through natural articulation of desired direction, scope and limits—not a detached binary checkpoint. Withdrawal or hesitation immediately suspends progression." },
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {sources.map((ref) => {
        ...
        return href ? (
          <a className="min-w-[190px] flex-1 rounded-xl border border-cam-gold/25 bg-background/55 p-3 transition hover:border-cam-gold/55" href={href} key={ref.id} rel={href.startsWith("http") ? "noreferrer" : undefined} target={href.startsWith("http") ? "_blank" : undefined}>{body}</a>
        ) : (
...
export default function RelationalGovernancePolished() {
  ...
  useEffect(() => setActivePhaseId("signals"), [activeRouteId]);
        ...
        <section className="overflow-hidden rounded-3xl border border-border/80 bg-card/45 shadow-sm" aria-labelledby="relational-runtime-heading">
          ...
          <div className="px-5 py-7 md:px-8 md:py-9">
            ...
            <div className="-mx-5 overflow-x-auto px-5 pb-4 md:-mx-8 md:px-8">
              <div className="flex min-w-[2320px] items-start">
                {runtimePhases.map((phase, index) => {
                  ...
                  return (
                    <div className="flex items-start" key={phase.id}>
                      <motion.article animate={{ opacity: 1, y: 0 }} className="cam-parchment-card flex shrink-0 flex-col overflow-hidden rounded-2xl border transition-colors duration-200" initial={{ opacity: 0, y: 10 }} layout style={{ width: isActive ? 560 : 240, minHeight: 320, borderColor: isActive ? GOLD : GOLD_BORDER, boxShadow: isActive ? `0 4px 24px rgba(184,147,90,0.15), 0 0 0 1px ${GOLD_BORDER}` : "0 1px 4px rgba(120,80,20,0.07)" }} transition={{ duration: 0.32, delay: index * 0.035 }}>
                        <button aria-expanded={isActive} className={`flex w-full flex-col p-5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring ${isActive ? "min-h-[220px]" : "h-[320px]"}`} onClick={() => setActivePhaseId(phase.id)} type="button">
                          <span className="mb-5 flex items-start justify-between gap-3"><span className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-cam-gold">{phaseOverride?.sublabel || phase.sublabel}</span><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-cam-gold/35 bg-[rgba(184,147,90,0.08)]"><ChevronRight className="h-3.5 w-3.5 text-cam-gold transition-transform" style={{ transform: isActive ? "rotate(90deg)" : "rotate(0deg)" }} /></span></span>
                            ...
                            <motion.div animate={{ opacity: 1, height: "auto" }} className="flex-1 overflow-hidden" exit={{ opacity: 0, height: 0 }} initial={{ opacity: 0, height: 0 }} transition={{ duration: 0.22 }}>
                              <div className="border-t border-cam-gold/25 px-5 pb-5 pt-5">
                                ...
                                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                                  <div className="rounded-xl border border-border/70 bg-background/55 p-4"><div className="mb-3 flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-cam-gold" aria-hidden="true" /><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-cam-gold">Runtime checks</p></div><ul className="grid gap-2.5">{activePhase.checks.map((item) => <li className="flex gap-2 text-sm leading-relaxed text-foreground/70" key={item}><Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-cam-gold" aria-hidden="true" /><span>{item}</span></li>)}</ul></div>
