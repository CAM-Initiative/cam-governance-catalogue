...
function KnowledgeCard({ record }: { record: LearnRecord }) {
  return <article className="cam-parchment-card flex h-full flex-col rounded-xl p-5 transition hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-md">
    <StatusBadges record={record} />
    <p className="mt-5 font-mono text-xs uppercase tracking-[0.16em] text-cam-gold">{record.id}</p>
    <h3 className="mt-2 font-serif text-2xl leading-snug text-foreground">{record.title}</h3>
    {record.caseDescriptor && <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{record.caseDescriptor}</p>}
    <p className="mt-4 text-base leading-relaxed text-foreground/85">{record.abstractedLearning}</p>
    {(record.canonicalFailureName || record.primaryFailureFamilyCode) && <div className="mt-5 border-t border-border/70 pt-4">
      <p className="report-label">Failure taxonomy</p>
      {record.canonicalFailureName && <p className="mt-1 font-serif text-lg leading-snug text-foreground">{record.canonicalFailureName}</p>}
    ...
    <div className="mt-5 flex flex-wrap gap-2">
      {record.knowledgeTags.slice(0, 4).map((tag) => <span key={tag} className="rounded-full border border-border bg-background/55 px-2.5 py-1 text-xs text-muted-foreground">{tag}</span>)}
    </div>
    <div className="mt-auto flex flex-wrap gap-3 pt-6">
      <Link href={`/observatory/knowledge-base/${encodeURIComponent(record.id)}`} className="inline-flex items-center rounded-lg bg-rose-900 px-4 py-2.5 font-mono text-xs uppercase tracking-[0.12em] text-rose-50 transition hover:bg-rose-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-700 focus-visible:ring-offset-2 focus-visible:ring-offset-background">Read lesson</Link>
      <Link href={`/observatory/reports/${encodeURIComponent(record.id)}`} className="inline-flex items-center rounded-lg border border-primary/30 bg-primary/10 px-4 py-2.5 font-mono text-xs uppercase tracking-[0.12em] text-[hsl(32_62%_25%)] transition hover:bg-primary/15">View evidence report</Link>
    </div>
  </article>;
}
...
export default function VigilKnowledgeBase() {
  ...
  return <Shell><main className="container mx-auto w-full max-w-7xl px-5 py-8 md:px-8 md:py-12 lg:px-10">
    {selectedId ? <>
      ...
      <section aria-label="Knowledge Base search and filters" className="cam-parchment-card mt-6 rounded-xl p-4 md:p-5">
        <label className="relative block"><span className="sr-only">Search lessons learned</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search lessons, failures, applications, vendors or record IDs" className="w-full rounded-lg border border-input bg-background/70 py-3 pl-10 pr-3 text-base text-foreground outline-none transition focus:border-primary/50 focus:ring-2 focus:ring-primary/15" /></label>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label><span className="report-label">Year</span><select value={year} onChange={(event) => setYear(event.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background/70 px-3 py-2.5 text-sm"><option value="">All years</option>{filterOptions.years.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
          <label><span className="report-label">Failure family</span><select value={failureFamily} onChange={(event) => setFailureFamily(event.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background/70 px-3 py-2.5 text-sm"><option value="">All failure families</option>{filterOptions.families.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
          <label><span className="report-label">Application</span><select value={application} onChange={(event) => setApplication(event.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background/70 px-3 py-2.5 text-sm"><option value="">All applications</option>{filterOptions.applications.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
          <label><span className="report-label">Monitoring</span><select value={monitoring} onChange={(event) => setMonitoring(event.target.value)} className="mt-1 w-full rounded-lg border border-input bg-background/70 px-3 py-2.5 text-sm"><option value="">All monitoring states</option><option value="required">Monitoring ongoing</option><option value="not-required">No monitoring declared</option></select></label>
        </div>
      </section>

      {state.status === "loading" && <div className="cam-parchment-card mt-6 rounded-xl p-6 text-muted-foreground">Loading published learning records…</div>}
      {state.status === "error" && <div className="cam-parchment-card mt-6 rounded-xl p-6"><p className="font-mono text-xs uppercase tracking-[0.16em] text-rose-800">Knowledge Base unavailable</p><p className="mt-3 text-muted-foreground">{state.message}</p></div>}
      {state.status === "ready" && <>
        {state.notice && <p className="mt-5 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm text-rose-900">{state.notice}</p>}
        <p className="mt-5 font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">{filtered.length} published lesson{filtered.length === 1 ? "" : "s"}</p>
        {grouped.length ? <div className="mt-6 space-y-10">{grouped.map(([groupYear, groupRecords]) => <section key={groupYear} aria-labelledby={`knowledge-year-${groupYear}`}><div className="flex items-center gap-4"><h2 id={`knowledge-year-${groupYear}`} className="font-serif text-3xl text-foreground">{groupYear}</h2><div className="h-px flex-1 bg-border" /></div><div className="mt-5 grid gap-5 lg:grid-cols-2">{groupRecords.map((record) => <KnowledgeCard key={record.id} record={record} />)}</div></section>)}</div> : <div className="cam-parchment-card mt-6 rounded-xl p-6"><p className="font-serif text-2xl text-foreground">No published lessons match this view</p><p className="mt-3 text-muted-foreground">Clear one or more filters, or check again after a VIGIL evidence chain has been closed with a published LEARN record.</p></div>}
      </>}
    </>}
  </main></Shell>;
}
