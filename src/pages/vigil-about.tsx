...
export default function VigilAbout() {
  return <Shell><VigilObservatoryNav /><main className="vigil-about-page"><div className="container mx-auto max-w-[1220px] px-4 py-8 sm:px-6 md:px-10 md:py-11">
    ...
    <section className="vigil-about-section" aria-labelledby="vigil-method-heading">
      ...
      <p className="vigil-about-record-intro">The seven sections below are the canonical public Case File and report structure. Sections 01–06 present the evidence-to-repair investigation; Section 07 provides the references required to audit the evidence, record chain and repair provenance. VIGIL record routing remains conditional, so an investigation may enter the lifecycle at different points and does not require every record type.</p>
      <div className="vigil-about-flow-scroll" role="region" aria-label="VIGIL seven-section public Case File and report model" tabIndex={0}>
        <div className="vigil-about-flow">
          {VIGIL_PUBLIC_REPORT_SECTIONS.map((section) => <article key={section.number}><span>Section {section.number}</span><h3>{section.label}</h3><p>{section.description}</p></article>)}
        </div>
      </div>
